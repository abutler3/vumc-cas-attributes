...
class CasAttributeSubscriber implements EventSubscriberInterface {
  ...
  public static function getSubscribedEvents() {
    $events[CasHelper::EVENT_PRE_REGISTER][] = array('onPreRegister', -10);
    $events[CasHelper::EVENT_PRE_LOGIN][] = array('onPreLogin', 20);
    return $events;
  }
  ...
  public function onPreRegister(CasPreRegisterEvent $event) {
    ...
    if ($this->settings->get('role.deny_registration_no_match')) {
      $roles = $this->doRoleMapCheck($event->getCasPropertyBag()->getAttributes());
      if (empty($roles)) {
  ...
  public function onPreLogin(CasPreLoginEvent $event) {
    ...
    $this->requestStack->getCurrentRequest()->getSession()->set('cas_attributes_properties', serialize($property_bag));
  ...
  protected function getFieldMappings() {
    ...
    foreach ($mappings as $field_name => $attribute_token) {
      $result = trim($this->tokenService->replace($attribute_token, [], ['clear' => TRUE]));
  ...
  protected function mapRoles(UserInterface $account, CasPreLoginEvent $event) {
    $roles = $this->doRoleMapCheck($event->getCasPropertyBag()->getAttributes());
    if ($this->settings->get('role.deny_no_match') && empty($roles)) {
  ...
  protected function doRoleMapCheck(array $attributes = NULL) {
    ...
    foreach ($role_map as $condition) {
      ...
      switch ($condition['method']) {
        ...
        case 'includes':
          foreach ($attr as $value) {
            if (strpos($this->strtolower($value), $this->strtolower($condition['value'])) !== FALSE) {
              $roles_added[] = $condition['rid'];
              continue 2;
            }
          }
